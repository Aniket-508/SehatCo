...
class InverseCookingModel(nn.Module):
    ...
    def forward(
        self, img_inputs, captions, target_ingrs, sample=False, keep_cnn_gradients=False
    ):
        ...
        if not self.recipe_only:
            ...
            target_one_hot_smooth[
                target_one_hot_smooth == 0
            ] = self.label_smoothing / target_one_hot_smooth.size(-1)
            ...
            # cardinality penalty
            losses["card_penalty"] = torch.abs(
                (ingr_probs * target_one_hot).sum(1) - target_one_hot.sum(1)
            ...
            losses["eos_loss"] = mult * (eos_loss * eos_pos.float()).sum(1) / (
                eos_pos.float().sum(1) + 1e-6
            ) + mult * (eos_loss * eos_head.float()).sum(1) / (
                eos_head.float().sum(1) + 1e-6
            )
            # iou
        ...
        outputs, ids = self.recipe_decoder(
            target_ingr_feats, target_ingr_mask, captions, img_features
        )
    ...
    def sample(
        self, img_inputs, greedy=True, temperature=1.0, beam=-1, true_ingrs=None
    ):
