...
def load_and_resize(root, path, imscale):
    ...
    img = Image.open(
        os.path.join(root, path[0], path[1], path[2], path[3], path)
    ).convert("RGB")
    img = transform(img)
...
def main(args):
    ...
    for split in ["train", "val", "test"]:
        datasets[split] = pickle.load(
            open(
                os.path.join(args.save_dir, args.suff + "recipe1m_" + split + ".pkl"),
                "rb",
            )
        )

        parts[split] = lmdb.open(
            os.path.join(args.save_dir, "lmdb_" + split), map_size=int(MAX_SIZE)
        )
        ...
        for i, entry in tqdm(enumerate(datasets[split])):
            ...
            for n, p in enumerate(impaths):
                ...
                if p.encode() not in present_entries:
                    im = load_and_resize(
                        os.path.join(args.root, "images", split), p, args.imscale
                    )
...
def test(args):
    ...
    for k, v in paths.items():
        path = k
        break
    image_file = lmdb.open(
        os.path.join(args.save_dir, "lmdb_" + "val"),
        max_readers=1,
        readonly=True,
        lock=False,
        readahead=False,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--suff", type=str, default="", help="id of the vocabulary to use"
    )
