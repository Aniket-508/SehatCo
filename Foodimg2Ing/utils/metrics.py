...
def compute_metrics(ret_metrics, error_types, metric_names, eps=1e-10, weights=None):
    ...
    if "f1" in metric_names:
        ...
        if "f1_ingredients" not in ret_metrics.keys():
            ...
            ret_metrics["f1_ingredients"].append(
                np.average(f1_perclass, weights=weights)
            )

        pre = error_types["tp_all"] / (
            error_types["tp_all"] + error_types["fp_all"] + eps
        )
        rec = error_types["tp_all"] / (
            error_types["tp_all"] + error_types["fn_all"] + eps
        )
