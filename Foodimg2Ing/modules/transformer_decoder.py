...
import Foodimg2Ing.modules.utils as utils
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(
        self, embed_dim, n_att, dropout=0.5, normalize_before=True, last_ln=False
    ):
        super().__init__()
        ...
        self.layer_norms = nn.ModuleList(
            [LayerNorm(self.embed_dim) for i in range(num_layer_norm)]
        )
    ...
    def forward(self, x, ingr_features, ingr_mask, incremental_state, img_features):
        ...
        if ingr_features is None:
            ...
            mask = torch.cat(
                (
                    torch.zeros(
                        img_features.shape[1], img_features.shape[0], dtype=torch.uint8
                    ).to(device),
                    ingr_mask,
                ),
                1,
...
class DecoderTransformer(nn.Module):
    ...
    def __init__(
        ...
        if self.normalize_inputs:
            self.layer_norms_in = nn.ModuleList(
                [LayerNorm(embed_size) for i in range(3)]
            )
    ...
    def forward(
        self, ingr_features, ingr_mask, captions, img_features, incremental_state=None
    ):
        ...
        for p, layer in enumerate(self.layers):
            x = layer(x, ingr_features, ingr_mask, incremental_state, img_features)
            ...
            for rem in range(len(sequences)):
                ...
                if not replacement:
                    ...
                    if i == 0:
                        ...
                        # ensure no repetitions in sampling if replacement==False
                        batch_ind = [
                            j for j in range(fs) if sequences[rem][0][i][j] != 0
                        ]
                ...
                for bid in range(beam):
                    ...
                    if indices[:, bid].item() == last_token_value:
                        ...
                        all_candidates.append(
                            [tokens, score, incremental, False, sequences[rem][-1] + 1]
                        )

            # if all the top-k scoring beams have finished, we can return them
            ordered_all = sorted(
                all_candidates + finished,
                key=lambda tup: tup[1] / (np.power(tup[-1], alpha)),
                reverse=True,
            )[:k]
